...
uint256 hashGenesisBlock("0x00000914f530682c4220dc8a68ef7c0fa96fc2ddb15ef26c10b8b749bce78d85");
...
const string strMessageMagic = "CaliphCoin Signed Message:\n";
...
{
    // CaliphCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
// Legacy
static const int64 nTargetTimespan = 6 * 60 * 60;
...
{
    // CaliphCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xde, 0xc1, 0x9e, 0xe0 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CaliphCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CaliphCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CaliphCoinMiner(CWallet *pwallet)
{
    printf("CaliphCoinMiner not supported!\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("CaliphCoin-miner");
    ...
    {
        printf("CaliphCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CaliphCoinMiner, pwallet));
}
